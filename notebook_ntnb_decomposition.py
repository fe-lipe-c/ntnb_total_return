...
URL_PYIELD_DATA = "https://github.com/crdcj/pyield-data/raw/refs/heads/main/data"
ANBIMA_PARQUET = "anbima_tpf.parquet"
VNA_PARQUET = "vna_ntnb.parquet"
VNA_BASE_CSV = "vna_base.csv"

df_anbima = pd.read_parquet(f"{URL_PYIELD_DATA}/{ANBIMA_PARQUET}")
df_ntnb = df_anbima.query('BondType == "NTN-B"')

df_vna = pd.read_parquet(f"{URL_PYIELD_DATA}/{VNA_PARQUET}")
df_vna_base = pd.read_csv(f"{URL_PYIELD_DATA}/{VNA_BASE_CSV}")
df_vna_base["reference_date"] = pd.to_datetime(df_vna_base["reference_date"]).dt.date
...
def get_coupon_payments(start_date, end_date, maturity_date):
    ...
    for _, row in relevant_cash_flows.iterrows():
        ...
        try:
            vna = df_vna_base.query("reference_date == @payment_date")["vna"].values[0]
        except IndexError:
            # If exact date not found, find the closest previous date
            closest_date = df_vna_base[df_vna_base["reference_date"] <= payment_date][
                "reference_date"
            ].max()
            vna = df_vna.query("reference_date == @closest_date")["vna_du"].values[0]
...
def decompose_ntnb_return(start_date, end_date, maturity_date):
    ...
    for i in range(len(dates_calculation) - 1):
        ...
        vna_start = df_vna.query("reference_date == @start_date_coupons")[
            "vna_du"
        ].values[0]
        vna_end = df_vna.query("reference_date == @end_date_coupons")["vna_du"].values[
            0
        ]

        # Get indicative rates for both dates
        rate_start = df_ntnb.query(
            "ReferenceDate == @start_date_coupons and MaturityDate == @maturity_date"
        )["IndicativeRate"].values[0]

        rate_end = df_ntnb.query(
...
end_date = pd.Timestamp("2024-12-30").date()
maturity_date = pd.Timestamp("2050-08-15").date()

inf_r, mm_r, y_r = decompose_ntnb_return(start_date, end_date, maturity_date)
math.prod(inf_r)
math.prod(mm_r)
math.prod(y_r)

(math.prod(y_r) * math.prod(mm_r) * math.prod(inf_r) - 1) * 100
math.prod(y_r) * math.prod(mm_r) * math.prod(inf_r) * 1000000

rate_start = df_ntnb.query(
    "ReferenceDate == @start_date and MaturityDate == @maturity_date"
)["IndicativeRate"].values[0]
...
for i in range(len(dates_calculation) - 1):
    ...
    vna_start = df_vna.query("reference_date == @start_date_coupons")["vna_du"].values[
        0
    ]
    vna_end = df_vna.query("reference_date == @end_date_coupons")["vna_du"].values[0]
